<script lang="ts">
	...
	import { javascript, scopeCompletionSource, snippets, localCompletionSource, completionPath } from "@codemirror/lang-javascript";
			...
			if (ScopeNodes.has(n.name)) {
				n.cursor().iterate(nn => {
					if (nn.name === 'VariableDeclaration') {
						...
						if (!objNode) return;
						const props = objNode.getChildren('Property').map(v => v.getChildren('PropertyDefinition')).reduce((p, c) => [...p, ...c]);
						for (const p of props) {
							const propId = context.state.doc.sliceString(p.from, p.to);
							console.log(propId);
							options.push({ label: '.' + propId, displayLabel: propId, type: 'property' });
						}
					}
				})
			}
	...
	export let src = '';
	export let changefunc = (src: string) => {};
</script>
...
<CodeMirror
	...
	class="editor-root"
	on:change={v => changefunc(v.detail)}
/>
